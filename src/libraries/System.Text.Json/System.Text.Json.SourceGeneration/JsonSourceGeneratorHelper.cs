...
{{
    ...
    {{
        ...
        {{
            get
            {{
                _{typeFriendlyName} ??= KnownCollectionTypeInfos<{valueTypeCompilableName}>.Get{collectionType}({valueTypeReadableName}, this);
...
{{
    ...
    {{
        ...
        {{
            get
            {{
                _{typeFriendlyName} ??= KnownDictionaryTypeInfos<{keyTypeCompilableName}, {valueTypeCompilableName}>.Get{collectionType}({valueTypeReadableName}, this);
                return _{typeFriendlyName};
            }}
        }}
    }}
        ...
        public TypeMetadata GetOrAddTypeMetadata(Type type)
        {
            ...
            else if (type.IsEnum || type.IsNullableValueType())
            {
...
");
        ...
        private static string GenerateDeserializeFunc(TypeMetadata typeMetadata)
        {
            ...
            sb.Append($@"
            private {typeCompilableName} DeserializeFunc(ref Utf8JsonReader reader, ref ReadStack readStack, JsonSerializerOptions options)
            {{
            ");
